...
def run_scheduled_sync() -> None:
    ...
    # A atualização ao vivo e a sincronização completa podem disparar no mesmo
    # instante. A completa é pouco frequente e não deve ser perdida por alguns
    # segundos de contenção; aguarda a coleta leve terminar.
    lock_acquired = scheduler_lock.acquire(timeout=90)
...
def run_scheduled_live_sync() -> None:
    ...
    try:
        ...
        logger.info(
            ...
            "fontes=%s | salvos=%s | degradada=%s | falhas=%s",
            execution["sync_run_id"],
            ",".join(execution["successful_sources"]),
            execution["saved"],
            execution["degraded"],
            execution.get("failures") or {},
        )
    except Exception as error:
        logger.exception(
...
def run_scheduled_backfill() -> None:
    ...
    if not backfill_lock.acquire(blocking=False):
        logger.info("Backfill adiado: o worker anterior ainda está ativo.")
        return
    session = None
    try:
        session = SessionLocal()
        result = MultiProviderSyncService(session).run_backfill(
            triggered_by="scheduler"
        )
        logger.info(
            "Backfill concluído: requests=%s estatísticas=%s lotes=%s falhas=%s",
            result.get("requests_used"), result.get("statistics_saved"),
            result.get("batches_completed"), result.get("failures") or (),
        )
    except Exception as error:
...
def run_scheduled_odds_sync() -> None:
    ...
    if settings.sync_provider != "multi_provider":
        return
    if not odds_sync_lock.acquire(blocking=False):
        logger.info("Coleta de odds adiada: o worker anterior ainda está ativo.")
        return
    session = None
    try:
        session = SessionLocal()
        result = MultiProviderSyncService(session).run_odds_refresh(
            triggered_by="scheduler"
        )
        logger.info(
            "Odds atualizadas: observações=%s salvas=%s previsões=%s falhas=%s",
            result.get("odds"), result.get("saved"),
            result.get("predictions_refreshed"), result.get("failures") or {},
        )
    except Exception as error:
